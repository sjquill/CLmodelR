...
child_recr = odin::odin({
  ...
  alpha <- user(2) #changes degree of assortativity in mixing across social classes (1 -> infinity, totally assortative -> totally proportional)
  x[,] <- user(0)
  y[,] <- user(0)
  ...
  arrest_w <- user(0.02) #rate of arrest bc of county lines involvement for the working group
  #lets say i want them working about 1 year, so 52 time steps, so 1/52
  arrest_h <- user(0.08) #rate of arrest bc of county lines involvement for the harm group
  #workign 3 months, so 12 time steps, so 1/12
  ...
  H_1_ini[] <- user(0) #number initially harming
  H_2_ini[] <- user(0) #number initially harming
  H_3_ini[] <- user(0) #number initially harming
  J_1_ini[] <- user(0) #number initially in jail
  J_2_ini[] <- user(0) #number initially in jail
  J_3_ini[] <- user(0) #number initially in jail

  initial(U_1[]) <- N_1_ini/(N_gender) - (W_1_ini[i] + H_1_ini[i] +J_1_ini[i])
  initial(U_2[]) <- N_2_ini/(N_gender) - (W_2_ini[i] + H_2_ini[i] +J_2_ini[i])
  initial(U_3[]) <- N_3_ini/(N_gender) - (W_3_ini[i] + H_3_ini[i] +J_3_ini[i])
  initial(W_1[]) <- W_1_ini[i]
  initial(W_2[]) <- W_2_ini[i]
  initial(W_3[]) <- W_3_ini[i]
  initial(H_1[]) <- H_1_ini[i]
  initial(H_2[]) <- H_2_ini[i]
  initial(H_3[]) <- H_3_ini[i]
  ...
  dim(H_1_ini) <- N_gender
  dim(H_2_ini) <- N_gender
  dim(H_3_ini) <- N_gender
  ...
  N[] <- U_1[i] + U_2[i] + U_3[i] + U_1[i] + W_2[i] + W_3[i] + H_1[i] + H_2[i] +H_3[i] + J_1[i] + J_2[i] + J_3[i]
  N_1[] <- U_1[i] + W_1[i] + H_1[i] + J_1[i]
  N_2[] <- U_2[i] + W_2[i] + H_2[i] + J_2[i]
  N_3[] <- U_3[i] + W_3[i] + H_3[i] + J_3[i]
  U[] <- U_1[i] + U_2[i] + U_3[i]
  W[] <- W_1[i] + W_2[i] + W_3[i]
  H[] <- H_1[i] + H_2[i] +H_3[i]
  J[] <- J_1[i] + J_2[i] + J_3[i]
  ...
  births_1[] <- mortality*N_1[i] + m*h[i]*H_1[i] #sum of all mortality <- births to maintain stable pop
  births_2[] <- mortality*N_2[i] + m*h[i]*H_2[i]
  births_3[] <- mortality*N_3[i] + m*h[i]*H_3[i]

  ##differential equations - INCLUSION
  deriv(U_1[]) <- births_1[i] + desist*(W_1[i]+H_1[i]) + (1-red)*release*J_1[i] + d*h[i]*W_1[i]*sum(mix_h[,i]) - contact*recruit_1[i]*U_1[i]*sum(mix1_r[,i]) - mortality*U_1[i]
  deriv(W_1[]) <- contact*recruit_1[i]*U_1[i]*sum(mix1_r[,i]) - h[i]*W_1[i]*sum(mix_h[,i]) - arrest_w*W_1[i] - desist*W_1[i] - mortality*W_1[i]
  deriv(H_1[]) <- (1-d-m)*h[i]*W_1[i]*sum(mix_h[,i]) + red*release*J_1[i] - m*h[i]*H_1[i]*sum(mix_h[,i]) - arrest_h*H_1[i] - desist*H_1[i] - mortality*H_1[i]
  deriv(J_1[]) <- arrest_w*W_1[i] + arrest_h*H_1[i] - release*J_1[i] - mortality*J_1[i]

  ##differential equations - EXCLUSION
  deriv(U_2[]) <- births_2[i] + desist*(W_2[i]+H_2[i]) + (1-red)*release*J_2[i] + d*h[i]*W_2[i]*sum(mix_h[,i]) - contact*recruit_2[i]*U_2[i]*sum(mix2_r[,i]) - mortality*U_2[i]
  deriv(W_2[]) <- contact*recruit_2[i]*U_2[i]*sum(mix2_r[,i]) - h[i]*W_2[i]*sum(mix_h[,i]) - arrest_w*W_2[i] - desist*W_2[i] - mortality*W_2[i]
  deriv(H_2[]) <- (1-d-m)*h[i]*W_2[i]*sum(mix_h[,i]) + red*release*J_2[i] - m*h[i]*H_2[i]*sum(mix_h[,i]) - arrest_h*H_2[i] - desist*H_2[i] - mortality*H_2[i]
  deriv(J_2[]) <- arrest_w*W_2[i] + arrest_h*H_2[i] - release*J_2[i] - mortality*J_2[i]

  ##differential equations - CLOSE PROXIMITY
  deriv(U_3[]) <- births_3[i] + desist*(W_3[i]+H_3[i]) + (1-red)*release*J_3[i] + d*h[i]*W_3[i]*sum(mix_h[,i]) - contact*recruit_3[i]*U_3[i]*sum(mix3_r[,i]) - mortality*U_3[i]
  deriv(W_3[]) <- contact*recruit_3[i]*U_3[i]*sum(mix3_r[,i]) - h[i]*W_3[i]*sum(mix_h[,i]) - arrest_w*W_3[i] - desist*W_3[i] - mortality*W_3[i]
  deriv(H_3[]) <- (1-d-m)*h[i]*W_3[i]*sum(mix_h[,i]) + red*release*J_3[i] - m*h[i]*H_3[i]*sum(mix_h[,i]) - arrest_h*H_3[i] - desist*H_3[i] - mortality*H_3[i]
  deriv(J_3[]) <- arrest_w*W_3[i] + arrest_h*H_3[i] - release*J_3[i] - mortality*J_3[i]

  #test of key CL dynamics
  test_prev_CL[] <- (W[i]+H[i])/N[i]
  test_incarcerated[] <- J[i]

  #making the mixing matrix
  ...
  prop1_r[] <- II[i]*((W_1[i]+H_1[i])/N_1[i]) + IE[i]*((W_2[i]+H_2[i])/N_2[i]) + IC[i]*((W_3[i]+H_3[i])/N_3[i])
  prop2_r[] <- EI[i]*((W_1[i]+H_1[i])/N_1[i]) + EE[i]*((W_2[i]+H_2[i])/N_2[i]) + EC[i]*((W_3[i]+H_3[i])/N_3[i])
  prop3_r[] <- CI[i]*((W_1[i]+H_1[i])/N_1[i]) + CE[i]*((W_2[i]+H_2[i])/N_2[i]) + CC[i]*((W_3[i]+H_3[i])/N_3[i])
  prop_h[] <- H[i]/N[i]
  mix1_r[,] <- prop1_r[i]*x[j,i]
  mix2_r[,] <- prop2_r[i]*x[j,i]
  mix3_r[,] <- prop3_r[i]*x[j,i]
  mix_h[,] <- prop_h[i]*y[j,i]
...
pars <- list(x = rbind(c(0.8, 0.2), #first row is group 1's mixing group 1,2,3,4,5
                       c(0.2, 0.8)),
             y = rbind(c(0.8, 0.2), #first row is group 1's mixing group 1,2,3,4,5
                       c(0.8, 0.2)),
             alpha = 1.5,
